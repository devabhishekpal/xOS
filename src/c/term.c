            ...
            if(term_curr_row == max_row){
                ...
                for(x = 0; x < VGA_WIDTH; x++){
                    if(is_gui_active()){
                        ...
                        const size_t indx = (VGA_HEIGHT - 1) * VGA_WIDTH + x;
                        term_buffer[indx] = vga_entry(' ', term_curr_color);
                    }
                }
            }
        }
        gui_set_default_console_curr_row_col(term_curr_row, term_curr_col);
...
void term_clear_last_char(void){
    if(is_gui_active()){
        ...
        int term_new_col = term_curr_col - 1;
